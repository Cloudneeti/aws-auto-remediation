...
stack_detail="$(aws cloudformation describe-stacks --stack-name cn-awsmultirem-functions-$env-$acc_sha --region $aws_region 2>/dev/null)"
stack_status=$?
...
aws cloudformation delete-stack --stack-name cn-awsmultirem-functions-$env-$acc_sha --region $aws_region 2>/dev/null
lambda_status=$?
