'''
dynamodb sub-orchestrator function
'''
...
def lambda_handler(event, context):
    ...
    if not PolicyId:
        ...
        try:
            Region = event["Region"]
            table_name = event["DynamodbTableName"]
        ...
        try:
            ...
            dynamodb = boto3.client('dynamodb', aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key,aws_session_token=aws_session_token,region_name=Region)  
        except ClientError as e:
            print(e)
            return {  
                'statusCode': 400,
        ...
        try:
            if PolicyId == "DynamoDbContinuousBackup":  
                responseCode,output = dynamodb_continuous_backups.run_remediation(dynamodb,table_name)
