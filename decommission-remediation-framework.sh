...
.EXAMPLE
    Command to execute : bash decommission-remediation-framework.sh [-a <12-digit-account-id>] [-p <primary-deployment-region>] [-e <environment-prefix>] [-s <list of regions where remediation is to enabled>]
...
.INPUTS
    ...
    **Mandatory(-p)AWS Region: Region where you want to deploy all major resources of remediation framework
    ...
    (-s)Region list: Comma seperated list(with no spaces) of the regions from where the remediation is to be decommissioned(eg: us-east-1,us-east-2)
        **Pass "all" if you want to decommission remediation from all other available regions
...
usage() { echo "Usage: $0 [-a <12-digit-account-id>] [-p <primary-deployment-region>] [-e <environment-prefix>] [-s <list of regions where remediation is to enabled>]" 1>&2; exit 1; }
env="dev"
version="1.0"
