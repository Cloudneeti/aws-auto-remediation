...
.EXAMPLE
    Command to execute : bash deploy-remediation-framework.sh [-a <12-digit-account-id>] [-e <environment-prefix>] [-v <1.0>] [-m <list of regions where remediation is to enabled>]
...
usage() { echo "Usage: $0 [-a <12-digit-account-id>] [-e <environment-prefix>] [-v <1.0>] [-m <list of regions where remediation is to enabled>]" 1>&2; exit 1; }
env="dev"
version="1.0"
regionlist=('all')
